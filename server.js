...
// ─── JSON BODY PARSER (10mb, skip webhook route) ──────────────────────────────
app.use((req, res, next) => {
...
async function rateLimitLogin(req, res, next) {
  ...
  try {
    ...
    if (!error) {
      if (count >= MAX) {
        return res.status(429).json({ error: `Too many login attempts. Try again in ${WINDOW_MINUTES} minutes.` })
      }
...
// ─── MULTER — strict memory storage, 50MB envelope ───────────────────────────
let _multer = null
try {
  _multer = require('multer')
} catch (_) {
  console.warn('multer not installed — /api/drive/upload will be unavailable. Run: npm install multer')
}

const memUpload = _multer
  ? _multer({ storage: _multer.memoryStorage(), limits: { fileSize: 50 * 1024 * 1024 } })
  : null
    ...
    try {
      ...
      if (jti) {
        const { data: revoked } = await supabase
          ...
          .eq('jti', jti)
          .single()
        if (revoked) return res.status(401).json({ error: 'Session revoked. Please sign in again.' })
      }
    } catch (_) {}
...
const requireContributor = (req, res, next) =>
  ['super_admin', 'admin', 'grant_coordinator', 'compliance_mgr', 'team_member'].includes(req.user?.role)
    ? next()
    : res.status(403).json({ error: 'Contributor access required — contact your administrator' })

const requireDelete = (req, res, next) =>
  ['super_admin', 'admin'].includes(req.user?.role)
    ? next()
    : res.status(403).json({ error: 'Delete requires Admin or Super Admin access' })
...
global._hasRecordType          = false
global._hasRecordId            = false
global._hasUserId              = true
global._safeActivityCols       = 'id,action,created_at'
...
async function refreshSchemaCache() {
  ...
  const [hasDetails, hasMetadata, hasRecordType, hasRecordId, hasUserId] = await Promise.all([
    testCol('details'),
    testCol('metadata'),
    testCol('record_type'),
    testCol('record_id'),
...
async function safeInsertLog(payload) {
  ...
  if (error?.message) {
    ...
    if (changed) {
      ...
      console.warn('safeInsertLog self-healed. New safe cols:', global._safeActivityCols)
...
app.post('/api/refresh-schema', auth, requireAdmin, async (req, res) => {
  ...
  res.json({
    success:        true,
    details_column: !global._detailsColumnMissing ? 'accessible' : 'missing — using metadata column fallback (OK)',
...
app.post('/api/login', rateLimitLogin, async (req, res) => {
  try {
    ...
    if (!data?.session?.access_token) { console.error('No session token for:', cleanEmail); return res.status(500).json({ error: 'Authentication failed — no session returned. Check Supabase email confirmation settings.' }) }

    const { data: profile, error: pe } = await supabase.from('user_profiles').select('*').eq('id', data.user.id).single()
    if (pe || !profile) { console.error('Profile not found for:', data.user.id); return res.status(401).json({ error: 'User profile not found. Contact administrator.' }) }
...
app.get('/api/applications', auth, async (req, res) => {
  ...
  let q = supabase.from('applications').select(
    '*, company:companies(id,company_name), wib:wib_records(id,wib_name,state), funding_opportunity:funding_opportunities(id,opportunity_name), revenue:revenue_records(fee_model,calculated_success_fee,invoice_status)',
    { count: 'exact' }
  )
  if (status) q = q.eq('status', status)
...
app.get('/api/notes', auth, async (req, res) => {
  ...
  let q = supabase.from('activity_log').select(baseCols + userJoin).eq('action', 'NOTE')
...
app.post('/api/notes', auth, async (req, res) => {
  ...
  const { data, error } = await safeInsertLog({
    ...
    details:  content,
    metadata: { note_type, is_aircall, content },
  })
  if (error) return res.status(400).json({ error: error.message })
  ...
  const { data: full } = await supabase.from('activity_log').select(baseCols2 + userJoin2).eq('id', data.id).single()
...
app.get('/api/tasks', auth, async (req, res) => {
  ...
  let q = supabase.from('activity_log').select(baseCols + userJoin).eq('action', 'TASK')
  if (record_id && global._hasRecordId !== false) q = q.eq('record_id', record_id)
  q = q.order('created_at', { ascending: false }).limit(Math.min(+limit, 500))
  const { data, error } = await q
  if (error) return res.status(400).json({ error: error.message })
...
app.put('/api/tasks/:id', auth, async (req, res) => {
  ...
  const newMeta   = { ...(existingParsed?.metadata || {}), ...req.body }
  const updatePayload = global._hasMetadata
    ? { metadata: newMeta }
    : { details: JSON.stringify({ text: existingParsed?.metadata?.title || existingParsed?.metadata?.text, ...newMeta }) }
...
app.get('/api/activity', auth, async (req, res) => {
  ...
  const baseCols = global._safeActivityCols || 'id,action,created_at'
  const userJoin = global._hasUserId !== false ? ',user:user_profiles!user_id(full_name,email)' : ''
  let q = supabase.from('activity_log').select(baseCols + userJoin).neq('action', 'NOTE').neq('action', 'TASK')
  if (record_type && global._hasRecordType !== false) q = q.eq('record_type', record_type)
  if (record_id   && global._hasRecordId   !== false) q = q.eq('record_id',   record_id)
...
app.get('/api/audit', auth, requireAdmin, async (req, res) => {
  ...
  const { data, error, count } = await supabase
    .from('activity_log')
    .select(baseCols + userJoin, { count: 'exact' })
    .order('created_at', { ascending: false })
...
app.put('/api/contacts/:id', auth, async (req, res) => {
  ...
  const updateVal = global._hasMetadata
    ? { metadata: merged, details: name || existingParsed?.metadata?.name }
    : { details: JSON.stringify({ text: name, ...merged }) }
...
app.get('/api/contracts', auth, async (req, res) => {
  ...
  const q = supabase.from('activity_log').select('*, user:user_profiles!user_id(full_name)').eq('action', 'CONTRACT').order('created_at', { ascending: false }).limit(+limit)
  const { data, error } = await q
  if (error) return res.status(400).json({ error: error.message })
  res.json({ data })
})
...
app.post('/api/import/:type', async (req, res) => {
  ...
  // ── Parse body (express.json already ran above for non-webhook routes) ──
  ...
  // ── Bulk-insert helper with single-row fallback ───────────────────────────
  ...
  try {
    ...
    if (type === 'wibs') {
      const wibStatusMap = {
        'funding available': 'funding_available', 'funding available - have program': 'funding_available',
        'funding_available': 'funding_available', 'open': 'funding_available', 'active': 'funding_available',
        ...
        'no reachout completed': 'no_reachout_complete', 'no reachout complete': 'no_reachout_complete',
        'no_reachout_complete': 'no_reachout_complete', 'new': 'no_reachout_complete', 'not contacted': 'no_reachout_complete',
        'funding not available': 'funding_not_available', 'funding_not_available': 'funding_not_available',
        'closed': 'funding_not_available', 'not applicable': 'no_reachout_complete',
        'stop applications': 'stop_applications', 'stop_applications': 'stop_applications',
        'closed - deadline': 'funding_not_available', 'closed - out of funds': 'funding_not_available',
      }
      ...
      const getWibField = (row, ...keys) => {
        ...
        for (const k of keys) {
          const found = Object.keys(row).find(rk =>
            rk.toLowerCase().replace(/[^a-z]/g, '').includes(k.toLowerCase().replace(/[^a-z]/g, ''))
          )
      ...
      for (const row of rows) {
        ...
        const stateFromName = (() => { const m = name.match(/^([A-Z]{2})\s*-\s*/); return m ? m[1] : null })()
      ...
      for (let i = 0; i < valid.length; i += 100) {
        await bulkInsert('wib_records', valid.slice(i, i + 100))
      }
    ...
    // ════════════════════════════════════════════════════════════════════════
      ...
      for (const row of rows) {
        ...
        if (notesParts.length) {
          insertRow.notes = notesParts.join('\n').substring(0, 10000)
          if (insertRow.notes.length === 10000) insertRow.notes += '...'
        }
        ...
        if (existingCo?.[0]) {
          ...
          if (error) {
            ...
            if (results.errors.length === 1) console.error('First company import error:', error.message)
          } else {
            results.created++
          }
        }
      ...
      for (const row of rows) {
        ...
        if (!name) {
          if (results.errors.length < 3) {
            results.errors.push(`DEBUG — CSV headers: ${Object.keys(row).slice(0, 8).join(', ')}`)
          }
          results.errors.push('Skipped — no location name')
          continue
        }
        ...
        const locRow = {
          ...
          status:         locStatusMap[rawStatus] || 'prospect',
          employee_count: (row['employee_count'] || row['Employee Count']) ? parseInt(row['employee_count'] || row['Employee Count']) || null : null,
      ...
      for (let i = 0; i < locBatch.length; i += 200) {
        ...
        if (error) {
          for (const r of chunk) {
            const { error: e2 } = await supabase.from('locations').insert(r)
            if (e2) results.errors.push(`"${r.location_name || '?'}": ${e2.message}`)
            else results.created++
          }
        } else {
    ...
    // ════════════════════════════════════════════════════════════════════════
      ...
      for (const row of rows) {
        const name = (
          ...
          row['Name']             || row['Record']           || row['Title']               ||
          row['Program Name']     || row['opportunity name'] || row['funding opportunity']  ||
          row['name']             || row['record']
        )?.trim() || ''
        if (!name) {
          if (results.errors.length < 2) {
            results.errors.push(`Skipped row — Opportunity Name required. CSV headers found: ${Object.keys(row).slice(0, 10).join(', ')}`)
          } else {
        ...
        valid.push({
          ...
          status:                        fundingStatusMap[rawS] || 'open',
          ...
          max_award_per_ein:             (row['max_award_per_ein']        || row['Max Award/EIN']  || row['Max per EIN'])     ? parseFloat(row['max_award_per_ein']       || row['Max Award/EIN'] || row['Max per EIN'])    || null : null,
          max_award_per_employee:        (row['max_award_per_employee']   || row['Max per employee']) ? parseFloat(row['max_award_per_employee']  || row['Max per employee']) || null : null,
      ...
      for (let i = 0; i < valid.length; i += 500) {
        await bulkInsert('funding_opportunities', valid.slice(i, i + 500))
      }
    ...
    // ════════════════════════════════════════════════════════════════════════
      ...
      const extractCompanyFromRecord = (record) => {
        ...
        for (const code of stateCodes) {
          for (const pat of [' - ' + code + '-', ' - ' + code + ' ', ' - ' + code + ',']) {
      ...
      const findCompanyId = async (name) => {
        ...
        try {
          const { data: d1 } = await supabase.from('companies').select('id').ilike('company_name', lower.substring(0, 20) + '%').limit(1)
          if (d1?.[0]) { companyMap.set(lower, d1[0].id); return d1[0].id }
      ...
      const knownAppKeys = new Set(['Record ID','Record','Status','Stage','Company','Company Name','Employer','Account','Account Name','WIB','Workforce Board','WIB Name','Board','Notes','Description','Comments','Award Requested','Amount Requested','Award Approved','Amount Approved','Application Approved Amount','Submission Date','Submitted','Decision Date','Decision','Created','Updated','Owner','Record Stage'])
      ...
      for (const row of rows) {
        ...
        const wibName    = rawWib || (recordName.split(' - ')[1] || '').trim() || null
        const wib_id     = wibName ? findWibId(wibName) : null

        if (!company_id && companyName) {
          results.errors.push(`Warning: "${companyName}" not matched to a Company`)
        }
        ...
        const getAmt  = (...keys) => { for (const k of keys) { const v = row[k]; if (v && String(v).trim()) return parseFloat(String(v).replace(/[$, ]/g, '')) || null } return null }
        const getDate = (...keys) => { for (const k of keys) { const v = row[k]; if (v && String(v).trim()) return String(v).trim().split('T')[0] } return null }

        const noteParts = []
        if (recordName) noteParts.push(`Application: ${recordName}`)
        const notesVal = row['notes'] || row['Notes'] || row['Description'] || row['Comments']
        if (notesVal) noteParts.push(notesVal)
        const extras = Object.entries(row).filter(([k, v]) => !knownAppKeys.has(k) && v && String(v).trim())
        ...
        if (error) {
          ...
          if (results.errors.length === 1) console.error('First app import error:', error.code, error.message, JSON.stringify(insertRow))
        } else {
          results.created++
        }
      }
...
// ─── CSV ESCAPE HELPER ────────────────────────────────────────────────────────
/**
 * Wrap a value in RFC-4180-compliant double quotes.
 * - Prepends a single quote to formula-injection prefixes (=, +, -, @, tab, CR/LF).
 * - Escapes any embedded double-quote characters by doubling them.
 */
const esc = (v) => {
...
app.get('/api/export/:type', auth, async (req, res) => {
  ...
  // ── Audit export ──────────────────────────────────────────────────────────
  if (type === 'audit') {
    const auditSelect = 'action,created_at'
      + (global._hasRecordType ? ',record_type' : '')
  ...
  // ── Streaming paginated export for all other types ────────────────────────
...
app.get('/api/template/:type', auth, (req, res) => {
  ...
  if (!csv) return res.status(400).json({ error: `Unknown template type: ${req.params.type}` })
  res.setHeader('Content-Type', 'text/csv')
...
app.post('/api/webhooks/aircall',
  ...
  async (req, res) => {
    ...
    const { data: upserted, error: upsertErr } = await supabase
      .from('aircall_calls')
      .upsert(upsertPayload, { onConflict: 'call_id', ignoreDuplicates: false })
      .select()
      .single()
    ...
    if (event === 'call.ended' && upserted.duration && !upserted.note_id) {
      ...
      const recording   = upserted.recording_url ? `\nRecording: ${upserted.recording_url}` : ''
      const noteContent = [`[AIRCALL NOTE] | ${callDate} | ${durationStr} | ${agentName}`, `Direction: ${direction}`, `Duration: ${durationStr}`, recording].filter(Boolean).join('\n')
...
app.post('/api/ai', auth, async (req, res) => {
  ...
  if (!apiKey) {
    return res.json({ text: 'AI Assistant requires an ANTHROPIC_API_KEY environment variable. Add it in your Render dashboard, then redeploy.', error: true })
  }
...
function broadcastPresence() {
  ...
  for (const clients of _sseClients.values()) {
    for (const r of clients) { try { r.write(payload) } catch (_) {} }
  }
}
...
app.get('/api/chat/users', auth, async (req, res) => {
  const { data, error } = await supabase.from('user_profiles').select('id,full_name,email,role').eq('is_active', true).neq('id', req.user.id).order('full_name')
  if (error) return res.status(400).json({ error: error.message })
  res.json({ data: data || [] })
})

app.get('/api/dm/unread-counts', auth, async (req, res) => {
  const { data, error } = await supabase.from('chat_dm_messages').select('sender_id').eq('recipient_id', req.user.id).eq('is_read', false)
  if (error) return res.status(400).json({ error: error.message })
  const counts = {}
...
app.post('/api/dm/:userId/read', auth, async (req, res) => {
  const me    = req.user.id
  const other = req.params.userId
  const { error } = await supabase.from('chat_dm_messages').update({ is_read: true }).eq('recipient_id', me).eq('sender_id', other).eq('is_read', false)
  if (error) return res.status(400).json({ error: error.message })
  res.json({ ok: true })
})
...
// ── Drive upload — strict multer memoryStorage, 50MB, multipart/related ────────
app.post('/api/drive/upload', auth, async (req, res) => {
  ...
  memUpload.single('file')(req, res, async (err) => {
    ...
    // Build multipart/related body entirely in memory using Buffer.concat
    const boundary = 'valorcrmboundary' + Date.now()
...
app.post('/api/drive/export', auth, async (req, res) => {
  ...
  const csvRows = [config.headers.map(esc).join(',')]
  let offset = 0
  while (true) {
  ...
  try {
    ...
    const boundary = 'valorexportboundary' + Date.now()
...
app.put('/api/users/:id/territories', auth, requireAdmin, async (req, res) => {
  ...
  try {
    const tRes  = territory_ids.length
      ? await supabase.from('territories').select('name').in('id', territory_ids)
      : { data: [] }
    const tNames = (tRes.data || []).map(t => t.name).join(', ') || 'none'
...
app.get('*', (req, res) => {
  ...
  res.status(503).send(`<!DOCTYPE html><html><head><title>Valor CRM</title></head>
<body style="font-family:sans-serif;background:#0B1E3C;color:#fff;padding:40px;text-align:center">
<h2>🛡️ Valor CRM</h2>
<h3 style="color:#C9A84C">Deployment Issue</h3>
<p>The application interface (index.html) was not found on the server.</p>
<p style="font-size:13px;color:rgba(255,255,255,.6)">
  Upload both <strong>server.js</strong> and <strong>index.html</strong> to the GitHub repository,
  then wait ~60 seconds for Render to redeploy.
</p>
<p style="margin-top:20px;font-size:11px;color:rgba(255,255,255,.4)">API is running. Only the frontend is missing.</p>
</body></html>`)
})
...
app.listen(PORT, () => {
  ...
  console.log('   📂 DRIVE UPLOAD: multer memoryStorage — 50MB envelope (multipart/related)')
